...
std::vector<pile*> deck::getPiles() {
	...
	for (int i = 0; i < totalFoundationPiles; i++) {
		pile* fPile = new foundationPile(sf::Vector2i(700 + i * (pileCard::WIDTH + 15), 30));
		piles.push_back(fPile);
	}
	for (int i = 0; i < totalTableauPiles; i++) {
		pile* tPile = new tableauPile(sf::Vector2i(150 + i * (pileCard::WIDTH + 40), 280));
		for (int j = 0; j <= i; j++) {
	...
	borrowPile* bPile = new borrowPile(sf::Vector2i(350, 30));
	pile* stPile = new stockPile(bPile, sf::Vector2i(150, 30));
